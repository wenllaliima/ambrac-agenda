/* ============================================================
   AMBRAC Agenda de Visitas — main.js
...
function getFiltered() {
  ...
  return allRecords.filter(r => {
    ...
    if (search) {
      const hay = [r.razao_social, r.cnpj, r.cidade, r.grupo,
                   r.unidade, r.visita, r.autor, r.endereco].join(' ').toLowerCase();
...
function renderCards() {
  ...
  document.getElementById('uberTotal').textContent =
    uberTotal > 0 ? `Uber: R$ ${uberTotal.toFixed(2).replace('.', ',')}` : '';
...
function cardHTML(r) {
  ...
  if (r.visitas_feitas) {
    ...
    if (u === 'OK')                                                       visitaClass = 'row-ok';
    else if (u.includes('REAG'))                                          visitaClass = 'row-reag';
    else if (u.includes('NÃO') || u.includes('NAO') || u.includes('DESMARC')) visitaClass = 'row-nao';
  }
  ...
  if (r.email) {
    ...
    emailRow = `<div class="card-row">
      <span class="row-icon">&#9993;</span>
      <span class="row-text row-email">${esc(emails[0])}</span>${more}
    </div>`;
  }
  ...
  return `
    <div class="card" data-id="${esc(r.id)}" tabindex="0" role="button" aria-label="${esc(r.razao_social)}">
      ...
      <div class="card-rows">
        ...
        ${emailRow}
        ${r.visitas_feitas ? `<div class="card-row ${visitaClass}"><span class="row-icon">&#10003;</span><span class="row-text">${esc(r.visitas_feitas)}</span></div>` : ''}
        <div class="card-row ${psicClass}"><span class="row-icon">&#129504;</span><span class="row-text">Psico: ${esc(r.psicossocial || 'pendente')}</span></div>
      </div>
...
async function deleteRecord() {
  ...
  if (error) { alert('Erro ao arquivar:\n' + error.message); return; }
  closeModal();
  if (pendentesMode) await loadPendentes(); else await loadDay(currentDate);
}
...
// ---- Run import: apaga por data antes de inserir ----
async function runImport() {
  ...
  // 1. Apagar todas as linhas das datas presentes no arquivo
  ...
  for (const date of dates) {
    ...
    if (error) {
      alert(`Erro ao limpar data ${date}:\n` + error.message);
...
// ============================================================
const FILL_COL_MAP = {
  'ORDEM': 'ordem', 'CATEGORIA': 'categoria', 'CLASSIFICAÇÃO': 'classificacao',
  'CNPJ': 'cnpj', 'GRUPO': 'grupo', 'RAZÃO SOCIAL': 'razao_social',
  'UNIDADE': 'unidade', 'SERVIÇOS EM CONTRATO': 'servicos_contrato',
  'DATA DA DOCUMENTAÇÃO': 'data_documentacao', 'PSICOSSOCIAL': 'psicossocial',
  'STATUS DA DOCUMENTAÇÃO': 'status_documentacao', 'AUTOR': 'autor', 'GRAZI': 'grazi',
...
function fillFormFromCSV(text) {
  const lines = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n').filter(l => l.trim());
  if (lines.length < 2) { showFillFeedback('Arquivo precisa ter cabeçalho e dados', true); return; }
  ...
  if (filled === 0) showFillFeedback('Nenhum campo reconhecido. Verifique o modelo.', true);
  else showFillFeedback(`${filled} campos preenchidos`, false);
}
